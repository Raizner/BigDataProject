...
int main(int argc,char *argv[])
{
	...
	string pathForRightDir = (string)"/home/artyom/BigDataProject/Calibration/" + argv[1] + "x" + argv[2] + "/right/" ;
	string pathForLeftDir = (string)"/home/artyom/BigDataProject/Calibration/" + argv[1] + "x" + argv[2] + "/left/" ;
	boost::filesystem::create_directories(pathForRightDir);
	int a = boost::filesystem::create_directories(pathForLeftDir);
...
*/
  ...
  if(key == 's')
  {
   ...
   //sprintf(FileName, "[%s]PedLeft%d.jpg",pathForRightDir.c_str(),iFileName);
	FileName = pathForLeftDir + "PedLeft" + to_string(iFileName) + ".jpg";
